    ...
    return (
        <div className="w-[97%] mx-auto flex justify-between items-center mt-2 mb-2">
            <form className=" flex flex-row">
                <input type="text" placeholder="search here.." name="searchValue" id="searchField" className="bg-gray-100 rounded px-4 py-1 text-sm" />
            ...
            <div className="flex items-center gap-2">
                <div className="h-[30px] w-[30px] rounded-full border border-blue-500 overflow-hidden">
                    {localStorage.getItem("PROFILE") !== null ? (
                        ...
                        <IoPersonOutline alt="" className="h-full w-auto object-cover aspect-[1/1]" />
                    )}
                </div>
                <div className="flex flex-col">
                    <p className="text-sm text-space-cadet font-medium">{localStorage.getItem("FULL_NAME")}</p>
                    <p className="text-xs font-normal text-gray-500">{localStorage.getItem("EMAIL")}</p>
                </div>
            </div>
        </div>
    )
